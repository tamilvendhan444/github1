...
router.post('/scan', requireAuth('admin'), async (req, res) => {
  try {
    const parsed = scanSchema.parse(req.body);
    let token: string | null = null;
    if ('token' in parsed) token = parsed.token;
    if ('qrData' in parsed) {
      try {
    ...
    if (!token) return res.status(400).json({ error: 'Invalid QR data' });
    ...
    return res.json({ ok: true, orderId: order.id, scannedAt: (updated[0] as any).qrScannedAt });
  } catch (e: any) {
